...
AI×恋愛シミュレーションモバイルアプリのポートフォリオ作品。ヒロイン「花(hana)」と自由会話＋選択肢分岐のシナリオで関係を深める。MVP＋v2（公開デプロイ）＋v3（シナリオ管理画面・動的トーン・ステージ演出）まで実装済み。

## v3 で追加したもの

- **シナリオ管理画面** (`/admin`): シナリオの新規作成・編集・公開/非公開切替・削除。`isAdmin` フラグでガード
- **選択肢分岐のエピソード**: シナリオ → シーン → 選択肢の階層をDBで管理、好感度で分岐
- **シナリオ一覧＋ロック機能**: 好感度閾値を満たした話だけ遊べる
- **動的トーン切替**: 好感度に応じて花の口調が変化（6フェーズ）
- **CharacterStage**: チャット画面にアバター＋呼吸アニメ＋表情切替（返信内容に応じて喜ぶ/驚く/落ち込む）
